...
public class CoreCommand extends MinigameCommand {
    ...
    public List<String> tabCompletion(CommandSender sender, String alias, String[] args) throws IllegalArgumentException {
        ...
        if (args.length == 1) returns = Arrays.asList("games", "game", "reload", "players", "id", "teams", "team", "scoreboard", "worlds", "world");
            ...
            case "game":
                ...
                break;
            case "id":
                returns = Collections.singletonList("list");
                break;
            case "team":
                returns = Collections.singletonList("list");
                break;
            case "scoreboard":
                returns = Collections.singletonList("refresh");
                break;
            case "world":
        ...
        if (!args[args.length-1].equals("")) returns = returns.stream().filter(s -> s.startsWith(args[args.length-1])).collect(Collectors.toList());
        return returns;
    }
    ...
    private String team(CommandSender sender, String label, String[] args) {
        ...
        if (args.length > 1)
            switch(args[1]) {
                case "list":
                    message = "Teams: \n" + MiniGameCore.teamManager.getTeams().parallelStream().map(t -> "   " + t.getTeamIdentifier() + ": "
                            + t.getPlayers().parallelStream().map(p -> p.getPlayer().getDisplayName()).collect(Collectors.joining(", "))).collect(Collectors.joining("\n"));
                    break;
            }
        return message;
    }
    ...
    private String scoreboard(CommandSender sender, String label, String[] args) {
        ...
        if (args.length > 1)
            switch(args[1]) {
                case "refresh":
                    message = "OLD COMMAND, has no function, Scoreboards refreshed";
                    break;
            }
        return message;
    }
